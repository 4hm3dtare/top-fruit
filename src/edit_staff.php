...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_POST['password'])) {
        ...
        $sql = "UPDATE staff SET username = ?, password = ?, name = ?, email = ?, role = ?, tele = ?, salary_rate = ? WHERE id = ?";
        $stmt = mysqli_prepare($con, $sql);
        mysqli_stmt_bind_param($stmt, "ssssssdi", $username, $password, $name, $email, $role, $tele, $salary_rate, $id);
    } else {
        $sql = "UPDATE staff SET username = ?, name = ?, email = ?, role = ?, tele = ?, salary_rate = ? WHERE id = ?";
        $stmt = mysqli_prepare($con, $sql);
    ...
    if (mysqli_stmt_execute($stmt)) {
        ...
        $error = "Error updating staff: " . mysqli_error($con);
    }
}
?>
...
<body>
    <div class="admin-header">
        <h1 class="admin-title">Edit Staff</h1>
    </div>
        ...
        <form method="POST">
            ...
            <div class="form-group">
                ...
                <select id="role" name="role" required>
                    ...
                    <option value="barista" <?php echo $staff['role'] === 'barista' ? 'selected' : ''; ?>>barista</option>
                    <option value="inventory_manager" <?php echo $staff['role'] === 'inventor_manager' ? 'selected' : ''; ?>>inventory manager</option>
                </select>
            </div>

            <div class="form-group">
                <label for="tele">Telephone</label>
                <input type="tel" id="tele" name="tele" value="<?php echo htmlspecialchars($staff['tele']); ?>" required>
            </div>

            <div class="form-group">
                <label for="salary_rate">Salary Rate</label>
                <input type="number" step="1" id="salary_rate" name="salary_rate" value="<?php echo htmlspecialchars($staff['salary_rate']); ?>" required>
            </div>
